...
interface Bme280ExtraConfigProps {
  config: Map<string, string>
  onConfigChange: (config: Map<string, string>) => void
}

export default function Bme280ExtraConfig(props: Bme280ExtraConfigProps) {
  ...
  return (
    <Box>
      ...
      <Typography id="pressure-id-input">
        MySensors child sensor identifier for <strong>pressure</strong>
      </Typography>
      <TextField id="pressure-id" aria-labelledby="pressure-id-input" defaultValue="2"
                 type="number" InputProps={{ inputProps: { min: 1, max: 256 } }}
                 value={config.get("pressureChildSensorId")}
                 onChange={handleConfigChange("pressureChildSensorId")} variant="outlined" fullWidth />

      <Typography id="debug-id-input">
        MySensors child sensor identifier for <strong>debug</strong>
      </Typography>
      <TextField id="debug-id" aria-labelledby="debug-id-input" defaultValue="3"
                 type="number" InputProps={{ inputProps: { min: 1, max: 256 } }}
