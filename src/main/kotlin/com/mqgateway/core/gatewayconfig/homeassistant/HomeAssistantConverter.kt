      ...
      when (device.type) {
        DeviceType.RELAY -> {
          ...
          if (device.config[DEVICE_CONFIG_HA_COMPONENT].equals(LIGHT.value, true)) {
            ...
            val deviceClass =
              device.config[DEVICE_CONFIG_HA_DEVICE_CLASS]?.let { HomeAssistantSwitch.DeviceClass.fromValue(it) }
            ...
            listOf(
              HomeAssistantSwitch(
                ...
                commandTopic,
                true,
                RelayDevice.STATE_ON,
                RelayDevice.STATE_OFF,
                deviceClass,
              ),
            )
          }
        ...
        DeviceType.GATE ->
          listOf(
            HomeAssistantCover(
              ...
              deviceClass =
                if (device.config[DEVICE_CONFIG_HA_DEVICE_CLASS].equals(DeviceClass.GATE.name, true)) {
                  DeviceClass.GATE
                } else {
                  DeviceClass.GARAGE
                },
